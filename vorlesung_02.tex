...
\begin{proof}
  ...
  \begin{tikzcd}[row sep=tiny, column sep=normal]
    ...
    {} & \dots \\
    (s,x) \urar[mapsto]{f_1} & {} &
    (s_2,x_2) \ular[mapsto,swap]{f_2} \urar[mapsto]{f_3} & {} &
    \ldots \ular[mapsto] \urar[mapsto] & {} &
    (s',x') \ular[mapsto]
  \end{tikzcd}
  \end{center}
...
\begin{definition}[$k$-Skelett]
  ...
  die Triangulierung 
  \[ (X\subk i, i\leq k; X(f))\,. \] %TODO: Benennung ist hier inkonsistent
  ...
  \begin{center}
    \begin{tikzpicture}[scale=1.4]
      \path[line normal, filled]
        (0,0) node[dotnode] {} node[below] {$(0,0)$}
          -- node[below] {$\Delta_p$}
        (2,0) node[dotnode] {} node[below] {$(1,0)$}
          -- 
        (2,2) node[dotnode] {} node[above] {$(1,1)$}
          --
        (0,2) node[dotnode] {} node[above] {$(0,1)$}
          -- node[left] {$\Delta_q$} 
        cycle;
      \path[line normal, dashed] (0,0) -- (2,2);
      \node at (.6,1.7) {$\Delta_p\times\Delta_q$};
    \end{tikzpicture}
  \end{center}
\end{beispiel}
  ...
  \begin{center}
    \begin{tikzcd}[row sep=normal, column sep=small]
      {} & \coprod\Delta_n\times X\subk n 
        \ar{dl}[swap]{\tau} \ar{dr}{\coprod\theta_n =: \theta} & \\
...
\begin{proof}
  \begin{enumerate}[label=(\alph*)]
    ...
    \item Sei $r = \left( (\xi_1,\ldots,\xi_p),(\eta_1,\ldots,\eta_q \right)\in
      ...
      \marginpar{\Bild zur Veranschaulichung der Tupel für $\Delta_1\times
      \Delta_1$}
     ...
     \marginpar{Verwaltung von Übungsaufgaben?}
  \end{enumerate}
\end{proof}
